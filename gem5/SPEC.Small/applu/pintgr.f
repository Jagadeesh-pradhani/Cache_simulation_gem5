...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(I,J) REDUCTION(+:FRC1)
      do j = jbeg,jfin1
         do i = ibeg, ifin1
            frc1 = frc1 + (  phi1(i,j)
     >                     + phi1(i+1,j)
...
!$OMP END PARALLEL
      ...
      frc2 = 0.0d+00
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(I,K) REDUCTION(+:FRC2)
      do k = ki1, ki2-1
         do i = ibeg, ifin1
            frc2 = frc2 + (  phi1(i,k)
     >                     + phi1(i+1,k)
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(J,K) REDUCTION(+:FRC3)
      do k = ki1, ki2-1
         do j = jbeg, jfin1
            frc3 = frc3 + (  phi1(j,k)
     >                     + phi1(j+1,k)
