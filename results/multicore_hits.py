...
# Main function to plot L1 I and D cache hits for all 16 cores and print comparison results
def main(x_label, plot_label, convert):
    src = "newprot/"
    metrics = []
    # Generate the metric list for 16 cores
    for core_id in range(16):
        metrics.append(f"system.ruby.l1_cntrl{core_id}.L1Dcache.m_demand_hits")  # L1D for each core
        metrics.append(f"system.ruby.l1_cntrl{core_id}.L1Icache.m_demand_hits")  # L1I for each core
    ...
    print("\nComparison of 'new' vs 'old' values:")
    for i, metric in enumerate(metrics):
        old_value = data["old.txt"][i]
        new_value = data["new.txt"][i]
        if new_value > old_value:
            percentage_increase = ((new_value - old_value) / old_value) * 100 if old_value != 0 else float('inf')
            print(f"{metric}: New value is greater by {percentage_increase:.2f}% (Old: {old_value}, New: {new_value})")

# Run the main function
if __name__ == "__main__":
    x_label = "Hits"
    plot_label = "L1D and L1I hits for 16 cores"
